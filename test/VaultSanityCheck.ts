...
describe("Aurora - VaultSanityCheck", () => {
  ...
  it("NEAR #rollToNextOption changes current option", async () => {
    const NEXT_OPTION_ADDR = "0x5366316bB6cC27F33C88c27e03bD54757FC91E28";
    ...
    assert.equal(await Vault.nextOption(), NEXT_OPTION_ADDR);
    ...
    assert.equal(await Vault.currentOption(), NEXT_OPTION_ADDR);
    assert.equal(await Vault.nextOption(), NULL_ADDR);
  });

  it("AURORA #rollToNextOption changes current option", async () => {
    const NEXT_OPTION_ADDR = "0xcCDe3B57996C7771529feC52562e335C85366381";
    const Vault = await ethers.getContractAt(
      RibbonThetaVaultLogic.abi,
