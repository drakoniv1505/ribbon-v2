...
function behavesLikeRibbonOptionsVault(params: {
  ...
  describe(`${params.name}`, () => {
    ...
    before(async function () {
      ...
      //replace mainnet contracts
      //deploy WETH contract
    const weth = await (await ethers.getContractFactory("WETH9")).deploy("WETH9", {
        from: owner,
        contract: "WETH9",
    });
    ...
    console.log(`${await wethDeployment.symbol()} Balance of ${owner}: ${await wethDeployment.balanceOf(owner)} ${await wethDeployment.symbol()}`);
    ...
    describe("#rollToNextOption", () => {
      ...
      it("is not able to roll to new option consecutively without setNextOption", async function () {
        ...
        await expect(
          vault.connect(keeperSigner).rollToNextOption(optionPremium.toString())
